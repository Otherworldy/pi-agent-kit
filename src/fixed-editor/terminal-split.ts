...
interface TerminalSplitCompositorOptions {
  ...
  shouldBypassFixedCluster?: () => boolean;
}
...
export class TerminalSplitCompositor {
  ...
  private readonly shouldBypassFixedCluster: () => boolean;
  ...
  constructor(options: TerminalSplitCompositorOptions) {
    ...
    this.shouldBypassFixedCluster = options.shouldBypassFixedCluster ?? (() => false);
  ...
  hideRenderable(target: PatchedRenderable, shouldHide: () => boolean = () => true): void {
    if (this.patchedRenders.some((patch) => patch.target === target)) return;
    const originalRender = target.render.bind(target);
    this.patchedRenders.push({ target, originalRender, shouldHide });
    target.render = (width: number) => shouldHide() ? [] : originalRender(width);
  }
  ...
  jumpToRootBottom(): boolean {
    if (this.disposed || this.shouldSuspendFixedCluster() || this.scrollOffset === 0) return false;
  ...
  private jumpToRootTarget(targetLines: readonly number[], direction: "previous" | "next"): boolean {
    if (this.disposed || targetLines.length === 0 || this.shouldSuspendFixedCluster()) return false;
  ...
  requestRepaint(): void {
    if (this.disposed || this.shouldSuspendFixedCluster()) return;
  ...
  dispose(options: DisposeOptions = {}): void {
    ...
    if (this.originalClearOnShrink !== null && typeof this.tui.setClearOnShrink === "function") {
      this.tui.setClearOnShrink(this.originalClearOnShrink);
    }
  ...
  private getScrollableRows(): number {
    if (this.disposed || this.writing || this.renderingCluster || this.checkingOverlay || this.shouldSuspendFixedCluster()) {
  ...
  private renderScrollableRoot(width: number): string[] {
    ...
    if (this.shouldBypassFixedClusterNow()) {
      return this.originalRender(width);
    }
  ...
  private handleInput(data: string): { consume?: boolean; data?: string } | undefined {
    if (this.disposed || this.shouldSuspendFixedCluster()) return undefined;
  ...
  private write(data: string): void {
    if (this.disposed || this.writing || this.shouldSuspendFixedCluster()) {
  ...
  private shouldBypassFixedClusterNow(): boolean {
    try {
      return this.shouldBypassFixedCluster();
    } catch {
      return false;
    }
  }

  private shouldSuspendFixedCluster(): boolean {
    return this.hasVisibleOverlay() || this.shouldBypassFixedClusterNow();
  }
