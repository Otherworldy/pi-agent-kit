...
- Editor chrome
- Chrome layout (slot visibility, side, and order)
- Status bar (plugin statuses, project directory, git; four corners around the input)
...
Pi v0.84 fullscreen provides the fixed bottom editor and chat scrolling. The plugin hides Pi's native footer to avoid repeating model, context, project, and extension-status lines already represented by the chrome. `editorChrome` restyles the native input as a solid gray panel with a left `▌` rail colored by thinking level, or green in bash (`!`) mode, equal side insets, and a bottom meta line. Change slot visibility, side, and order in `/agent-kit` → Chrome layout, or set `agentKit.chrome.left` / `agentKit.chrome.right` (order = display order; omit a slot to hide it). Available slots: `model`, `thinking`, `timer` (session-total AI working duration, live while working, frozen when idle, resets on a new session), `tps` (streaming rate, e.g. `45.7 t/s`; freezes on the last rate when idle, `0 t/s` before the first stream), `ttft` (session-average request time to first streamed token, e.g. `450ms ttft` / `1.2s ttft`; `0ms ttft` before the first sample, frozen while idle, resets on a new session), `providerCompat`, `fast`, `context` (`34k/500k`), `cost`. Default: left `model · thinking · timer · tps · ttft · providerCompat · fast`, right `cost · context`. Other plugins' `setStatus` texts and string `setWidget` lines, plus this plugin's project directory and git summary, sit on lines **around** the input (top-left / top-right / bottom-left / bottom-right). Native widget rows above/below the editor are blanked so those texts are not duplicated. Same controls as Chrome layout (`/agent-kit` → Status bar; Space cycles the four corners then hidden). Default is `auto`: plugin statuses top-left (alphabetical), `projectDir` then `git` bottom-right. New unlisted plugin keys stay top-left; unlisted builtins stay bottom-right; `hidden` keys stay off. Legacy `{ left, right }` maps to the top corners. `showGitStatus: false` / `showProjectDir: false` still hide those builtins in auto mode. The working indicator stays on the bottom-left of that row. Matching sent-user-message styling is unchanged; Pi's native fullscreen dock handles compaction status.
